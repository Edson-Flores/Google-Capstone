# GOOGLE CASE STUDY: Bellabeat Fitness Data Analysis 
##### Author: Edson Flores
...
- Comprehensive: Data minute-level output for physical activity, heart rate, and sleep monitoring. While the sample size is small and most data is recorded during certain days of the week, there are varying reported data entries for each users where which may impact representation. 
- Current: Data is from March 2016 to May 2016. Data is not current so the users habit may be different now. 
- Cited: Unknown. 

⛔ The dataset has limitations:

- Only 30 user data is available. While 30 users may be an arguable number to accurately represent all Bellabeat users, we aren't fully aware of the total users.
- Upon further analysis to check for unique user Id, the set has 33 user data from daily activity, 24 from sleep and only 8 from weight. There are 3 extra users and some users did not record their data for tracking daily activity and sleep. 
- Some data is missing or users entered incomplete data.

## 3. Process

Examine the data, check for NA, and remove duplicates for three main tables: daily_activity and sleep_day:
```
dim(sleep_day)
sum(is.na(sleep_day))
